...
Install the JoinQuest Claude Code plugin (skill + MCP in one step). From the [developer dashboard](https://joinquest.cc/developers) → **Connect an AI assistant** → Claude Code tab, copy the install command with your API key filled in.
...
### Cursor (manual MCP only)

If you prefer not to use the plugin, paste into `.cursor/mcp.json`:
